# This is the main functions. called B1 
...
class Creature:
    ...
    def get_distance_travelled(self, fitness_type="combined", target_direction=(1, 0), weights=(0.7, 0.3)):
        # return 0 if either starting or ending position is undefined
        if self.start_position is None or self.last_position is None:
            return 0
        # convert the positions to NumPy arrays for vector operations
        p1 = np.asarray(self.start_position)
        p2 = np.asarray(self.last_position)

        if fitness_type == "vertical":
            # return positive vertical distance (z-axis)
            ...
            # to normalise the direction vector
            ...
            # split weights for vertical and horizontal 
            w1, w2 = weights
            # compute vertical progress
            vertical = max(p2[2] - p1[2], 0)
            # compute horizontal vector and to normalise the direction vector
            horizontal = np.array([p2[0] - p1[0], p2[1] - p1[1]], dtype=np.float64)
            dir_vec = np.array(target_direction, dtype=np.float64)
            dir_vec /= np.linalg.norm(dir_vec) if np.linalg.norm(dir_vec) > 0 else 1.0
            # compute horizontal progress
            horizontal_progress = max(np.dot(horizontal, dir_vec), 0)
            # return the weighted vertical and horizontal progress
    ...
    def get_max_height_achieved(self):
        # check the non empty position history
        if hasattr(self, 'position_history') and self.position_history:
            # get max Z-axis value from position history
            max_height = max(pos[2] for pos in self.position_history)
            # return the positive difference between max height and starting height
            return max(max_height - self.start_position[2], 0)
        # use vertical progress if no position available
