# Expand Waveform Library

# Introduce additional fundamental MotorType enums. Consider adding:

# TRIANGLE: Provides a linear ramp up and down.
# SAWTOOTH: Generates a signal that rises linearly and then sharply drops. 
# These new waveforms can lead to different kinds of continuous, controlled, or jerky movements.
...
class MotorType(Enum):
    ...
    TRIANGLE = 3
    # Linear rise, sharp drop
    SAWTOOTH = 4

class Motor:
    def __init__(self, control_waveform, control_amp, control_freq):
        # Select motor waveform type based on input value
        if control_waveform < 0.25:
            self.motor_type = MotorType.PULSE
        elif control_waveform < 0.5:
            self.motor_type = MotorType.SINE
        # 50% chance for waveform types
    ...
    def get_output(self):
        ...
        if self.motor_type == MotorType.PULSE:
            ...
            p_norm = self.phase / np.pi
            if p_norm < 1.0:
                output = 1.0 - 2.0 * p_norm
            else:
                output = -1.0 + 2.0 * (p_norm - 1.0)
        elif self.motor_type == MotorType.SAWTOOTH:
            # Sawtooth wave: rises then sharply drops
