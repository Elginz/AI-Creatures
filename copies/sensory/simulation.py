...
class Simulation: 
    ...
    def run_creature(self, cr, iterations=2400):
        ...
        if len(cr.get_expanded_links()) > 100:
            print(f"[SKIP] Creature has too many links ({len(cr.get_expanded_links())}), skipping.")
            return
        ...
        for step in range(iterations):
            ...
            # Update the creature's sensors with the latest orientation data
            cr.update_sensors(orn)
    ...
    def update_motors(self, cid, cr):
        ...
        # Get the current sensor value from the creature
        pitch_value = cr.get_pitch()

        for jid in range(p.getNumJoints(cid,
                                        physicsClientId=self.physicsClientId)):
            m = cr.get_motors()[jid]

            # Pass the sensor value to the motor
            velocity = m.get_output(pitch_value)
